...
const getUserProfile =async(req,res) =>{
    ...
    try {
        const user =await User.findOne({username}).select("-password").select("-updatedAd");
...
const loginUser = async (req, res) => {
    try {
        ...
        if (!user || !isPasswordCorrect) return res.status(400).json({ error: "Invaild username or password" });
...
const logoutUser = async (req, res) => {
    try {
        ...
        res.status(200).json({ error: "User logged out successfuly" });
    } catch (error) {
        res.status(500).json({ error: error.message });
        console.log("Error in loginUser: ", error.message);
    }
